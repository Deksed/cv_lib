#!/usr/bin/env python
"""
Compare multiple training runs: config parameters + best metrics side-by-side.

Reads train_config.json (saved by cv_lib.train) and results.csv (saved by
Ultralytics) from each run directory, then prints a comparison table.

Usage examples:

  # Compare two named run dirs:
  python scripts/compare_runs.py runs/train/exp1 runs/train/exp2

  # Scan a project dir for all runs automatically:
  python scripts/compare_runs.py --project runs/train

  # Show only specific config fields:
  python scripts/compare_runs.py runs/train/exp1 runs/train/exp2 --fields model_path epochs imgsz batch
"""
...
def _print_table(runs: list[Path], fields: list[str]) -> None:
    ...
    name_w = max(len(n) for n in run_names)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare training runs: configs + best metrics.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Provide run dirs directly:  compare_runs.py runs/train/exp1 runs/train/exp2\n"
            "Or scan a project dir:      compare_runs.py --project runs/train"
        ),
    )
    parser.add_argument(
