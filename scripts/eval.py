#!/usr/bin/env python
"""
Run model.val() on a dataset and print a mAP table + save confusion matrix.

Usage examples:

  # Basic — prints mAP table, saves confusion_matrix.png next to the yaml:
  python scripts/eval.py --model runs/train/best.pt --data dataset/data.yaml

  # Custom output path and confidence threshold:
  python scripts/eval.py --model best.pt --data data.yaml --cm-out eval/cm.png --conf 0.25

  # Suppress confusion matrix:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate a YOLO model and print mAP table.")
    ...
    # --- confusion matrix ---
    if not args.no_cm:
        cm_path = Path(args.cm_out) if args.cm_out else Path(args.data).parent / "confusion_matrix.png"
