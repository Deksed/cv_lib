...
├── data/                    # __init__ (YOLO parsing), inspect.py, convert.py, dvc_gen.py
...
`cvlib inspect|convert|cvat-query|compare|infer|eval|bench|compare-runs|dvc-init`.
...
- `data/split.py` — train/val/test split + data.yaml generation (also the `split`
  stage referenced by `dvc_gen` / `cvlib dvc-init`)
