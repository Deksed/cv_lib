...
├── data/                    # __init__ (YOLO parsing), inspect.py (health check), convert.py
...
`cvlib inspect|convert|cvat-query|compare|infer|eval|bench|compare-runs`.
...
- `data/split.py` — train/val/test split + data.yaml generation
